...
def get_user_by_id(user_id: uuid.UUID, db: Session):
    ...
    if entity:
        logging.info('User retrieved with user_id {}'.format(user_id))
    else:
...
def get_all_users(db: Session):
    ...
    logging.info('Retrieved all users, count: {}'.format(len(entities)))
    return entities
...
def update_user(user: User, changed_user: UserCreateSchema, db: Session):
    ...
    db.commit()
    db.refresh(user)
    logging.info('User updated with user_id {}'.format(user.id))
    return user
...
def delete_user_by_id(user_id: uuid.UUID, db: Session):
    ...
    if entity:
        db.delete(entity)
        db.commit()
        logging.info('User deleted with user_id {}'.format(user_id))
