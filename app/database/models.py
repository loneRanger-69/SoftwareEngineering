...
IN_STOCK = 'stock >= 0'
pizzaType_id = 'pizza_type.id'
greater_zero = 'quantity > 0'
...
class Sauce(Base):
    ...
    spiciness: Mapped[SauceSpiciness] = mapped_column(default=SauceSpiciness.LEVEL1, nullable=False)
    stock: Mapped[int] = mapped_column(CheckConstraint(IN_STOCK), nullable=False)

    def __repr__(self):
        return (
            f"Sauce(id='{self.id}', name='{self.name}', "
            f"price='{self.price}', description='{self.description}', "
            f"spiciness='{self.spiciness}')"
        )
...
class PizzaType(Base):
    ...
    toppings: Mapped[List['PizzaTypeToppingQuantity']] = relationship(cascade='all, delete-orphan',
                                                                      back_populates='pizza_type')  # NOSONAR
    sauces: Mapped[List['PizzaTypeSauceQuantity']] = relationship(cascade='all, delete-orphan',
                                                                  back_populates='pizza_type')  # NOSONAR
...
class PizzaTypeSauceQuantity(Base):
    __tablename__ = 'pizza_type_sauce_quantity'

    pizza_type_id: Mapped[uuid.UUID] = mapped_column(ForeignKey(pizzaType_id), primary_key=True)
    pizza_type: Mapped['PizzaType'] = relationship(back_populates='sauces')
    sauce_id: Mapped[uuid.UUID] = mapped_column(ForeignKey('sauces.id'), primary_key=True)
    quantity: Mapped[uuid.UUID] = mapped_column(Integer, CheckConstraint(greater_zero), nullable=False)
    sauce: Mapped['Sauce'] = relationship()

    def __repr__(self):
        return "PizzaTypeSauceQuantity(pizza_type_id='%s', sauce_id='%s', quantity='%s')" \
            % (self.pizza_type_id, self.sauce_id, self.quantity)
...
class Order(Base):
    ...
    beverages: Mapped[List['OrderBeverageQuantity']] = relationship(cascade='all, delete-orphan',
                                                                    backref='customer_order')  # NOSONAR
